...
import { ChevronLeft, ShoppingBag, Star, Share2, ShieldCheck, Heart, Truck } from "lucide-react";
  ...
  const handleAddToCart = () => {
    if (product) {
      const productWithSale = {
        ...product,
        flash_sale: flashSale ? { sale_price: flashSale.sale_price, end_time: flashSale.end_time } : undefined
      };
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
          ...
          >
            <div className="aspect-[4/5] overflow-hidden rounded-[40px] shadow-2xl relative">
              <img 
                ...
                alt={product.name} 
                className={cn("w-full h-full object-cover", !product.is_stock && "grayscale opacity-70")}
                referrerPolicy="no-referrer"
              />
              {!product.is_stock && (
                 <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                   <span className="bg-white text-ink px-10 py-4 rounded-full text-lg font-bold uppercase tracking-widest">Tạm hết hàng</span>
                 </div>
              )}
            </div>
            {product.images && product.images.length > 1 && (
              <div className="grid grid-cols-4 gap-4">
                {product.images.map((img, i) => (
                  <div key={i} className="aspect-square rounded-2xl overflow-hidden cursor-pointer hover:opacity-80 transition-opacity border border-border-beige">
          ...
          >
            <div className="mb-10">
              ...
              <h1 className="text-5xl md:text-6xl font-serif leading-tight mb-6">{product.name}</h1>
              <div className="flex items-center space-x-6 mb-8">
                <div className="flex items-center">
                  {[1,2,3,4,5].map(s => (
                    <Star key={s} className="w-4 h-4 fill-accent text-accent" />
                  ))}
                  <span className="ml-3 text-sm text-ink/40 font-medium">({reviews.length} đánh giá)</span>
                </div>
              ...
              <div className="flex items-baseline space-x-4 mb-10">
                <span className="text-4xl font-bold text-ink">{displayPrice.toLocaleString('vi-VN')}₫</span>
                {hasSale && (
                  <span className="text-xl text-ink/30 line-through font-light">{product.price.toLocaleString('vi-VN')}₫</span>
                )}
              </div>
              ...
              <div className="space-y-8 mt-auto">
                <div className="flex items-center space-x-6">
                  <div className="flex items-center bg-paper border border-primary/10 rounded-full px-6 py-3">
                    ...
                    <span className="mx-8 font-mono text-lg font-bold">{quantity}</span>
                    <button 
                  ...
                  <button 
                    ...
                    className={cn(
                      ...
                      isFavorite ? "text-red-500 fill-red-500" : "hover:text-red-500"
                    )}
                  >
               ...
               <div className="mt-auto">
                  <button disabled className="w-full bg-ink/10 text-ink/30 py-5 rounded-full font-bold uppercase tracking-widest text-sm cursor-not-allowed">
                    Hết hàng tạm thời
                  </button>
               </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-8 mt-16 p-8 bg-paper rounded-[32px] border border-primary/5">
               <div className="flex flex-col items-center text-center">
                 ...
                 <span className="text-[10px] uppercase tracking-widest font-bold opacity-40">Giao hàng</span>
                 <span className="text-sm font-medium mt-1">2 Giờ hỏa tốc</span>
               </div>
               <div className="flex flex-col items-center text-center">
                 <ShieldCheck className="w-8 h-8 text-primary mb-3 stroke-[1px]" />
                 <span className="text-[10px] uppercase tracking-widest font-bold opacity-40">Đảm bảo</span>
                 <span className="text-sm font-medium mt-1">Tươi mới cực hạn</span>
               </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-32">
          <div className="flex items-center space-x-12 border-b border-primary/5 mb-12 overflow-x-auto">
             ...
             >
               Chi tiết sản phẩm
               {activeTab === "description" && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-primary"></div>}
             </button>
             <button 
             ...
             >
               ...
               {activeTab === "reviews" && <div className="absolute bottom-0 left-0 w-full h-[2px] bg-primary"></div>}
             </button>
          </div>
            ...
            {activeTab === "description" ? (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <p className="text-ink/60 text-lg leading-relaxed font-light">
                  Những đóa hoa được các thợ thủ công lành nghề nhất của FLORA tuyển chọn khắt khe từng cành một. Chúng tôi cam kết sử dụng hoa tươi nhập khẩu trực tiếp từ các trang trại chuẩn GlobalGAP tại Đà Lạt và Hà Lan.
                </p>
                <div className="grid grid-cols-2 gap-x-12 gap-y-6">
                  <div>
                    <h5 className="font-serif text-lg mb-2">Thành phần</h5>
                    <p className="text-ink/40 text-sm">Hoa hồng Ecuador, Lá bạc nhập khẩu, Giấy gói lụa cao cấp.</p>
                  </div>
                  <div>
                    <h5 className="font-serif text-lg mb-2">Độ bền</h5>
                    <p className="text-ink/40 text-sm">Trung bình 5-7 ngày tùy điều kiện bảo quản.</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                {reviews.length > 0 ? reviews.map(review => (
                  <div key={review._id} className="border-b border-primary/5 pb-10 last:border-0">
                    <div className="flex items-center justify-between mb-4">
                      <h6 className="font-serif text-xl">{review.user_name || "Khách Hàng Ẩn Danh"}</h6>
                      <div className="flex items-center text-accent">
                        {[...Array(Math.round(review.rating))].map((_, i) => <Star key={i} className="w-4 h-4 fill-current" />)}
                      </div>
                    </div>
                    <p className="text-ink/60 font-light mb-6 leading-relaxed italic">"{review.comment}"</p>
                    {review.images && review.images.length > 0 && (
                      <div className="flex space-x-4">
                        {review.images.map((img, i) => (
                           <img key={i} src={img} className="w-24 h-24 object-cover rounded-2xl shadow-md" alt="review" />
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-12 bg-paper rounded-[40px] text-center border-2 border-dashed border-primary/10">
                    ...
                    <p className="text-ink/40 font-medium uppercase tracking-widest text-xs">Chưa có đánh giá nào cho sản phẩm này</p>
                    <p className="text-sm mt-2 text-ink/20">Hãy trở thành người đầu tiên chia sẻ cảm nhận!</p>
                  </div>
                )}
              </div>
            )}
